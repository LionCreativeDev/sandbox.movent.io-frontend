...
import { userComplianceService, ComplianceInvoice } from "@/lib/services/userComplianceService";
import { Badge, INVOICE_SC, fmtDate, paymentMethodText } from "@/components/compliance/shared";
...
export default function InvoicesModal({
    ...
    useEffect(() => {
        let cancelled = false;
        setLoading(true);
        userComplianceService.project.billing(projectId)
    ...
    const download = async (inv: ComplianceInvoice) => {
        ...
        try { await userComplianceService.project.invoiceDownload(inv.id, `${inv.invoice_number}.txt`); }
