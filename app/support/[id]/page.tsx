...
export default function StaffSupportTicketPage() {
  ...
  const canManage = can('client', 'canManageClientSupport');
  ...
  const load = () => {
    userSupportService.get(Number(id))
      .then(setData)
      .catch((err) => { if (!handleNotFound(err, router)) router.push('/support'); })
  ...
  const changeAssign = async (userId: string) => {
    ...
    try {
      await userSupportService.assign(Number(id), userId ? Number(userId) : null);
  ...
  const changeStatus = async (status: string) => {
    ...
    try {
      await userSupportService.updateStatus(Number(id), status);
    ...
    <DashboardLayout title={`Ticket #${t.id}`}>
      <div style={{ maxWidth: 760 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          <button onClick={() => router.push('/support')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 20 }}>←</button>
          <div style={{ flex: 1 }}>
        ...
        {t.project && (
          <div style={{
            ...
            <a href={`/projects/${t.project.id}`} style={{ color: '#2563eb', fontWeight: 600, textDecoration: 'none', fontSize: 12 }}>Open Project →</a>
          </div>
        )}
        ...
        {t.description && (
          <div style={{ background: '#fff', borderRadius: 10, border: '1px solid #e2e8f0', padding: 18, marginBottom: 16 }}>
            ...
            {t.attachment_name && (
              <button
                onClick={() => userSupportService.downloadAttachment(t.id, t.attachment_name)}
                ...
                return (
                  <div key={r.id} style={{ display: 'flex', flexDirection: isClient ? 'row' : 'row-reverse', gap: 10, marginBottom: 14 }}>
                    ...
                    <div style={{ maxWidth: '75%' }}>
                      ...
                      {r.attachment_name && (
                        <button
                          onClick={() => userSupportService.downloadReplyAttachment(t.id, r.id, r.attachment_name)}
