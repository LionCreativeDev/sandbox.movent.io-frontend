...
// canAssignTasks — mirrors Api\User\TaskController::assignedToRule().
const INTERNAL_ASSIGNEE_ROLES = ['production', 'developer', 'designer', 'qa'];
// Only a Client portal account can never be a task assignee. A Project
// Manager genuinely on the project's team CAN be one, and so can any Seller
// of the company since 2026-09-04 — Sellers come from their own list
// (tasks.assignableSellers()) rather than the project's team_members, since
// they never have a row there.
...
export default function CreateTaskPage() {
  ...
  // Sellers are the one exception to "this project's team only": any Seller
  // of the company is assignable (2026-09-04 request), and none of them has
  // a project_team_members row to be found in teamOptions above.
  ...
  useEffect(() => {
    ...
    userProjectService.getOne(projectId)
      .then(p => {
        ...
        // Company Sellers, listed separately below. Failure is silent: the
        // rest of the picker still works, it just won't offer Sellers.
        if (canAssignTasks) {
