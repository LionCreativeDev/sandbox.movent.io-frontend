...
export const SIMPLE_PROJECT_PERMISSIONS: SimpleProjectPermission[] = [
  ...
  {
    // canViewTasks used to live inside pm_manage_tasks below — the same
    // problem canEditTasks had before its own 2026-09-02 split, but sharper:
    // as of the 2026-09-04 worker-role change, Developer/Designer/Production/
    // Team Member hold canViewTasks and NONE of the manage keys. With the
    // view key still bundled, "Manage Tasks" showed unchecked for them
    // (collapse needs every mapped key), so the first time a Company Admin
    // saved their Edit User page, expand() would have silently dropped
    // canViewTasks too — taking away the one task right those roles are
    // meant to keep. It has to be its own checkbox to survive a round-trip.
    key: 'pm_view_tasks', label: 'View Tasks',
    maps: ['canViewTasks'],
  },
  {
    key: 'pm_manage_tasks', label: 'Manage Tasks',
    maps: ['canCreateTasks', 'canCreateLinkedProjectTask', 'canAssignTasks'],
  },
  {
