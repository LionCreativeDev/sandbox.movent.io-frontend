...
import { Recruitment, ApplicantStatus, RecruitmentStatus } from '@/lib/services/adminHrService';
...
export default function RecruitmentDetailPage() {
  ...
  const canUpdate = usePermission('hr', 'canUpdateRecruitment');
  const canManage = usePermission('hr', 'canManageRecruitment');

  const load = async () => {
    try { setPosting(await hrService.recruitment.getOne(recruitmentId)); }
  ...
  const addApplicant = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await hrService.recruitment.addApplicant(recruitmentId, { name, email: email || undefined, phone: phone || undefined });
  ...
  const updateStatus = async (applicantId: number, status: ApplicantStatus) => {
    ...
    try {
      await hrService.recruitment.updateApplicantStatus(recruitmentId, applicantId, status);
  ...
  const removePosting = async () => {
    ...
    setRemoving(true);
    try {
      await hrService.recruitment.remove(recruitmentId);
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    setSavingEdit(true);
    try {
      await hrService.recruitment.update(recruitmentId, {
