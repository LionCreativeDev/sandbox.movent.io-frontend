...
export default function ClientAttachmentsModal({
    ...
    useEffect(() => {
        let cancelled = false;
        setLoading(true);
        userComplianceService.project.clientAttachments(projectId)
    ...
    const download = async (m: ComplianceChatMessage) => {
        ...
        try { await userComplianceService.project.clientAttachmentDownload(m.id, m.attachment_name ?? "attachment"); }
