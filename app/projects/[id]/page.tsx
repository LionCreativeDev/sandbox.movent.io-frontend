...
import { ROLE_LABELS, roleDisplayLabel, canRoleReassignTasks } from "@/lib/roleUtils";
...
export default function UserProjectDetailPage() {
    ...
    // …except for the roles that may never move a task to someone else,
    // which that bypass would otherwise cover. Mirrors the same
    // NON_REASSIGNING_ROLES check update() enforces — see roleUtils.
                                            ...
                                            >
                                                {(() => {
                                                    const isSelfTask =
                                                        assignedToId(t) ===
                                                        me?.id;
                                                    // iCanReassign first: the
                                                    // isSelfTask/team-member
                                                    // paths below are
                                                    // permission-free, so a
                                                    // worker role would slip
                                                    // through them.
                                                    if (
                                                        !iCanReassign ||
                                                        (!canEditTasks &&
