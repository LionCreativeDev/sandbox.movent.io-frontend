...
import { adminHrService, AttendanceCalendar, Employee } from '@/lib/services/adminHrService';
...
export default function AttendanceMonthlyReportPage() {
  ...
  useEffect(() => {
    adminHrService.employees.list({ status: 'active' }).then(list => {
      setEmployees(list);
      const requested = new URLSearchParams(window.location.search).get('employee_id');
      if (requested) setEmployeeId(requested); // eslint-disable-line react-hooks/set-state-in-effect
      else if (list.length > 0) setEmployeeId(String(list[0].id)); // eslint-disable-line react-hooks/set-state-in-effect
    }).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    adminHrService.attendance.calendar(Number(employeeId), month, year)
      .then(setData)
