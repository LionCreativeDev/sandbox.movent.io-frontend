...
import GatewayPaymentStep, {
    GatewayChargeFields,
} from "@/components/billing/GatewayPaymentStep";
import toast from "react-hot-toast";
import {
    seatPurchaseService,
    SeatCatalog,
    TierOption,
} from "@/lib/services/seatPurchaseService";
...
function UpgradeSeatsContent() {
    ...
    const [type, setType] = useState<PurchaseType>(
        searchParams.get("type") === "seats" ? "seats" : "companies",
    );
    ...
    const tiers: TierOption[] =
        type === "seats"
            ? (catalog?.seat_tiers ?? [])
            : (catalog?.company_tiers ?? []);
    const current =
        type === "seats"
            ? (catalog?.current.max_users_per_company ?? null)
            : (catalog?.current.max_companies ?? null);
    ...
    const handlePurchase = async (
        ...
        fields: GatewayChargeFields,
    ) => {
        const admin = await seatPurchaseService.purchase({
            type,
            tier_value: selectedValue ?? null,
            gateway,
            ...fields,
        });
        ...
        <DashboardLayout title="Upgrade">
            <div style={{ width: "100%" }}>
                <div style={{ marginBottom: 20 }}>
                    ...
                    >
                        {type === "seats" ? "Upgrade Staff Seats" : "Upgrade Company Slots"}
                    </h1>
                    <p
                        style={{
                            margin: "4px 0 0",
                    ...
                    >
                        {type === "seats"
                            ? "Raise your account's staff seat / client-portal login limit"
                            : "Raise your account's company limit"}
                    </p>
                </div>

                <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "#f1f5f9", borderRadius: 10, padding: 4, maxWidth: 360 }}>
                    {(["companies", "seats"] as PurchaseType[]).map((t) => (
                        <button
                            key={t}
                            onClick={() => {
                                setType(t);
                        ...
                        >
                            {t === "seats" ? "Staff Seats" : "Company Slots"}
                        </button>
                    ))}
                </div>
                ...
                {loading ? (
                    ...
                    >
                        You're already on the <strong>Unlimited</strong> tier —
                        nothing to upgrade.
                    </div>
                ) : (
                    <>
