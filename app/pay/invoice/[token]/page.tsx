...
import InlineGatewayPayment, {
  InlineGatewayPaymentHandle,
} from "@/components/payments/InlineGatewayPayment";
...
const GATEWAY_META: Record<string, { icon: string; desc: string }> = {
  ...
  bank_transfer: {
    icon: "🏦",
    desc: "Direct bank deposit (1–3 business days)",
  },
};
...
function PublicInvoicePayContent() {
  ...
  const [selectedAccountId, setSelectedAccountId] = useState<number | null>(
    null,
  ); // the specific company_gateway_id to charge
  ...
  const markSeen = (messages: { id: number; is_guest: boolean }[]) => {
    ...
    try {
      window.localStorage.setItem(lastSeenKey, String(maxId));
    } catch {}
    setChatUnread(0);
  };
    ...
    const check = () => {
      publicInvoiceChatService
        ...
        .then((res) => {
          ...
          let lastSeen = 0;
          try {
            lastSeen = Number(window.localStorage.getItem(lastSeenKey) ?? 0);
          } catch {}
          const unread = res.messages.filter(
            (m) => !m.is_guest && m.id > lastSeen,
          ).length;
  ...
  useEffect(() => {
    ...
    axios
      .get(`${base}/public/invoices/${token}`)
      .then((res) => {
  ...
  const handlePay = async () => {
    ...
    if (selectedGateway === "bank_transfer") {
      ...
      try {
        ...
        setPayError(
          ex.response?.data?.message ?? "Submission failed. Please try again.",
        );
      } finally {
        setPaying(false);
      }
  ...
  const centreCard: React.CSSProperties = {
    ...card,
    textAlign: "center",
    padding: "56px 40px",
    maxWidth: 460,
          ...
          <>
            ...
            >
              <div>
                ...
                >
                  {fmtProjectDate(p.created_at ?? undefined)}
                </div>
              </div>
              <div>
                <div
              ...
              <div>
                ...
                <div
                  style={{
                    marginTop: 4,
                    fontSize: 13,
                    color: (p.outstanding ?? 0) > 0 ? "#ea580c" : "#059669",
                    fontWeight: 700,
                  }}
                >
                  {fmt(p.outstanding ?? 0, inv.currency)}
                </div>
              </div>
            </div>
                  ...
                  {projectInvoices.map((pi) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: "10px 12px",
                        borderRadius: 8,
                        background: pi.is_current ? "#eff6ff" : "#f8fafc",
                        border: `1px solid ${pi.is_current ? "#bfdbfe" : "#e2e8f0"}`,
                      }}
                    >
                      <div>
                        <div
                        ...
                        >
                          {cap(pi.status)}
                          {pi.due_date ? ` • Due ${fmtDate(pi.due_date)}` : ""}
                        </div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        ...
                        >
                          {fmt(pi.total_amount, pi.currency)}
                        </div>
                        <div
                          style={{
                            fontSize: 11,
                            fontWeight: 700,
                            color: pi.outstanding > 0 ? "#ea580c" : "#059669",
                            marginTop: 3,
                          }}
                        >
    ...
    return (
      <div style={card}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {history.map((p) => {
            const sc = PROJECT_STATUS_STYLE[p.status] || {
              bg: "#f1f5f9",
              color: "#64748b",
            };
      ...
      <div style={wrap}>
        <div style={inner}>
          <div style={centreCard}>
            ...
            <h2 style={{ margin: "0 0 10px", fontSize: 20, color: "#1e293b" }}>
            ...
            >
              ...
              <br />
              Please contact the sender for a valid link.
            </p>
          </div>
        </div>
      </div>
  ...
  if (pageState === "pending_confirmation" && invoice)
    return (
      <div style={wrap}>
        <div style={inner}>
          <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={centreCard}>
            ...
            <h2 style={{ margin: "0 0 8px", fontSize: 22, color: "#a16207" }}>
            ...
            >
              A payment for this invoice has already been submitted and is
              awaiting confirmation by <strong>{invoice.company_name}</strong>.
              You will be contacted once it&apos;s verified.
            </p>
            <div
              style={{
                background: "#f8fafc",
  ...
  if (pageState === "success" && invoice) {
    const gw =
      invoice.available_gateways.find((g) => g.id === selectedAccountId) ??
      invoice.available_gateways.find((g) => g.type === selectedGateway);
        ...
        <div style={inner}>
          ...
          <div style={{ ...card, textAlign: "center", padding: "48px 40px" }}>
            ...
            >
              {isBankTransfer ? (
                <>
                  Thank you, <strong>{invoice.customer_name}</strong>. Please
                  complete the bank transfer using the details provided.
                  <br />
            ...
            >
              {(
                [
                  ...
                  ...(isBankTransfer && gatewayRef
                    ? [["Reference", gatewayRef]]
                    : []),
                  ...(!isBankTransfer && onlinePaymentRef
                    ? [["Transaction ID", onlinePaymentRef]]
                    : []),
                ] as [string, string][]
              ).map(([l, v]) => (
                <div
            ...
            <p style={{ margin: "16px 0 0", fontSize: 12, color: "#94a3b8" }}>
              {isBankTransfer
                ? `Status will be updated after verification by ${invoice.company_name}`
                : "A receipt has been recorded for this payment."}
            </p>
          </div>
          {renderProjectCard(invoice)}
          {chatWidget}
  ...
  const chargeCurrency =
    selectedGw?.conversion_preview?.currency ?? invoice.currency;
      ...
      <div style={inner}>
        ...
        >
          <div>
            ...
            {invoice.due_date && (
              ...
              >
                {isOverdue ? "⚠️ Overdue — was due " : "Due "}
              ...
              {invoice.discount_amount > 0 && (
                ...
                >
                  ...
                  <span style={{ fontSize: 13, color: "#16a34a" }}>
                    − {fmt(invoice.discount_amount, invoice.currency)}
                  </span>
                </div>
              )}
              <div
        ...
        {invoice.available_gateways.length === 0 && (
          ...
          >
            ...
            <div style={{ fontSize: 13, color: "#ef4444" }}>
              {invoice.gateway_unavailable_message ??
                `Please contact ${invoice.company_name} to arrange payment.`}
            </div>
          </div>
        )}
          ...
          <div style={card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {invoice.available_gateways.map((gw) => {
                ...
                const active =
                  gw.id !== null
                    ? selectedAccountId === gw.id
                    : selectedGateway === gw.type;
                return (
                  <div key={gw.id ?? gw.type}>
                    <button
                      type="button"
                      onClick={() => {
                        setSelectedGateway(gw.type);
                        setSelectedAccountId(gw.id);
                      }}
                      style={{
                        display: "flex",
                        alignItems: "center",
                    ...
                    {gw.conversion_preview && (
                      ...
                      >
                        {gw.label} doesn&apos;t support {invoice.currency} —
                        you&apos;ll be charged{" "}
                        <strong>
                          {fmt(
                            gw.conversion_preview.amount,
                            gw.conversion_preview.currency,
                          )}
                        </strong>{" "}
                        for{" "}
                        {fmt(
                        ...
                        (1 {invoice.currency} ={" "}
                        {gw.conversion_preview.rate.toFixed(4)}{" "}
                        {gw.conversion_preview.currency}).
                      </div>
                    )}
                  </div>
        ...
        {isBankSelected && invoice.bank_details && (
          ...
          >
            ...
            >
              Use invoice number <strong>{invoice.invoice_number}</strong> as
              payment reference. Enter your transfer reference below after
              sending.
            </p>
          </div>
        )}
            ...
            <InlineGatewayPayment
              ...
              createOrderUrl={
                selectedGateway === "paypal"
                  ? `${base}/public/invoices/${token}/gateways/paypal/create-order`
                  : undefined
              }
          ...
          selectedGateway !== "paypal" && (
            <button
              ...
              style={{
                ...
                fontSize: 16,
                fontWeight: 700,
                cursor: paying || !selectedGateway ? "not-allowed" : "pointer",
                border: "none",
                background:
                  paying || !selectedGateway
                    ? "#93c5fd"
                    : isOverdue
                      ? "#dc2626"
                      : "#2563eb",
