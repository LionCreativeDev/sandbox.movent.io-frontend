...
import {
  ...
  userLeadService,
  Lead,
} from "@/lib/services/adminLeadService";
import {
  adminClientService,
  ClientCompany,
...
export default function LeadsPage() {
  ...
  useEffect(() => {
    load();
  }, [status, priority, companyF, query]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSearch = (e: React.SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    // Same text re-submitted: the effect won't fire, so run the load by hand.
    if (search === query) load();
        ...
        >
          <div>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#94a3b8" }}>
              {leads.length} total · {newCount} new · {wonCount} won ·{" "}
              {lostCount} lost
            </p>
          </div>
          {canCreate && (
            <button
              onClick={() => router.push(`${leadRoot}/new`)}
              style={{
                display: "flex",
                alignItems: "center",
                gap: 7,
          ...
          {STATUSES.map((s) => (
            <button
              ...
              style={{
                padding: "6px 14px",
                borderRadius: 20,
                border: status === s.key ? "none" : "1.5px solid #e2e8f0",
                fontSize: 12,
                fontWeight: 600,
                cursor: "pointer",
                background:
          ...
          >
            <div style={{ flex: "1 1 200px", position: "relative" }}>
              ...
              <input
                ...
                placeholder={
                  isMultiCompanyAdmin
                    ? "Search name, email, phone, organization, company…"
        ...
        >
          {loading ? (
            ...
            <div style={{ padding: 60, textAlign: "center", color: "#94a3b8" }}>
              ...
              <div
                style={{ fontWeight: 600, color: "#64748b", marginBottom: 4 }}
              >
                No leads found
              </div>
              <div style={{ fontSize: 13 }}>
                Start tracking your sales pipeline by adding a lead
              </div>
            </div>
          ) : (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {leads.map((lead, i) => {
                  ...
                  const statusKey = lead.converted_at
                    ? "converted"
                    : lead.status;
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        borderBottom:
                          i < leads.length - 1 ? "1px solid #f8fafc" : "none",
                        cursor: "pointer",
                      }}
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 14px" }}>
                        ...
                        >
                          {STATUS_LABEL[statusKey] ?? cap(statusKey)}
                        </span>
                      </td>
                      <td style={{ padding: "13px 14px" }}>
                        <span
                      ...
                      >
                        {lead.source ? cap(lead.source.replace("_", " ")) : "—"}
                      </td>
                      <td
                        style={{
                          padding: "13px 14px",
