...
import { userSupportService, SupportTicket, TICKET_CATEGORIES, TICKET_STATUSES, TICKET_PRIORITIES } from '@/lib/services/userSupportService';
...
export default function SupportTicketsPage() {
  const canView = can('client', 'canViewClientSupport') || can('client', 'canManageClientSupport');
  ...
  const load = (filters: { status?: string; category?: string; priority?: string; project_id?: string; search?: string }) => {
    ...
    userSupportService.list(Object.keys(params).length ? params : undefined)
      .then(setTickets)
      .catch(() => setTickets([]))
  ...
  useEffect(() => {
    if (canView) userProjectService.list().then(p => setProjects(p.map((pr: any) => ({ id: pr.id, name: pr.name })))).catch(() => setProjects([]));
  ...
  const applyFilters = (overrides: Partial<{ status: string; category: string; priority: string; project_id: string; search: string }> = {}) => {
    load({ status, category, priority, project_id: projectId, search, ...overrides });
  };
    ...
    return (
      <DashboardLayout title="Support Tickets">
        <div style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>
          You don&apos;t have permission to view client support tickets.
        </div>
      </DashboardLayout>
    );
  }
  ...
  return (
    <DashboardLayout title="Support Tickets">
      <h2 style={{ fontSize: 20, fontWeight: 700, color: '#1e293b', margin: '0 0 16px' }}>Client Support Tickets</h2>
