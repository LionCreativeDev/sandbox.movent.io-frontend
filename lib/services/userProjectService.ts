...
export const userProjectService = {
    ...
    tasks: {
        ...
        // The "Assigned To" picker's Seller options. Sellers are assignable
        // (2026-09-04) but never have a project_team_members row, so unlike
        // every other candidate they can't be read off the project payload's
        // team_members — hence their own ungated endpoint. Company-scoped and
        // active-only server-side; assignedToRule() re-validates whatever id
        // is actually submitted.
        assignableSellers: async (): Promise<
            { id: number; name: string; role_type?: string }[]
        > => (await api.get("/user/tasks/assignable-sellers")).data.data,
    },
