...
import { Employee } from '@/lib/services/adminHrService';
...
export default function EmployeeDetailPage() {
  ...
  const canEdit = usePermission('hr', 'canEditEmployees');
  const canDelete = usePermission('hr', 'canDeleteEmployees');
  const canDeactivate = usePermission('hr', 'canDeactivateEmployees');
  const canManageDocuments = usePermission('hr', 'canManageEmployeeDocuments');
  const canViewPayroll = usePermission('hr', 'canViewPayroll');

  const load = async () => {
    setLoading(true);
    try {
      setEmployee(await hrService.employees.getOne(employeeId));
    } catch (err) {
      if (!handleNotFound(err, router)) { toast.error('Failed to load employee'); router.push('/employees'); }
    }
  ...
  const deactivate = async () => {
    ...
    try {
      setEmployee(await hrService.employees.deactivate(employeeId));
  ...
  const reactivate = async () => {
    try {
      setEmployee(await hrService.employees.reactivate(employeeId));
  ...
  const deleteEmployee = async () => {
    ...
    try {
      await hrService.employees.remove(employeeId);
  ...
  const uploadDoc = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      await hrService.employees.documents.upload(employeeId, docFile, docTitle);
      ...
      <div style={{ maxWidth: 1000 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {canEdit && <Link href={`/employees/${employeeId}/edit`} style={{ padding: '9px 16px', border: '1px solid #e2e8f0', background: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600, color: '#1e293b', textDecoration: 'none' }}>Edit</Link>}
            {canDeactivate && (employee.status === 'terminated' ? (
                ...
                {employee.payrolls.map(p => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                    ...
                    <td style={{ padding: '10px 20px' }}>
                      {canViewPayroll && <Link href={`/payroll/${p.id}/payslip`} style={{ fontSize: 12, color: '#2563eb', fontWeight: 600, textDecoration: 'none' }}>View Payslip</Link>}
                    </td>
                  </tr>
                ))}
              </tbody>
