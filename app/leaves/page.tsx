...
import { LeaveRequestRecord, LeaveType, Employee } from '@/lib/services/adminHrService';
...
export default function LeavesPage() {
  ...
  const canApply = usePermission('hr', 'canApplyLeave');
  const canApprove = usePermission('hr', 'canApproveLeave');
  ...
  const load = async () => {
    ...
    try {
      ...
      setLeaves(await hrService.leaves.list(params));
    } catch (err: any) {
      if (err?.response?.status === 403) toast.error('You do not have permission to view leave requests');
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await hrService.leaves.create({ employee_id: Number(employeeId), leave_type: leaveType, from_date: fromDate, to_date: toDate, reason: reason || undefined });
  ...
  const decide = async (id: number, status: 'approved' | 'rejected') => {
    setBusyId(id);
    try {
      await hrService.leaves.updateStatus(id, status);
