      ...
      .then((b) => {
        ...
        if (b.country) {
          const match = ALL_COUNTRIES.find(
            (c) => c.name === b.country || c.code === b.country,
          );
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        country:
          ALL_COUNTRIES.find((c) => c.code === countryCode)?.name ?? null,
        ...
        ...(!editing && companies.length > 1 && companyId
      ...
      toast.error(
        errText(
          err,
          editing ? "Failed to update brand" : "Failed to create brand",
        ),
      );
    } finally {
      setSaving(false);
        ...
        <form onSubmit={submit} style={card}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>
              Shown in the &quot;Billed From&quot; block on this brand&apos;s
              invoices. No need to type https:// — it&apos;s added on save.
            </div>
          </div>
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 5 }}>
                  Any image up to 2 MB — PNG, JPG, WEBP, SVG, GIF, HEIC and so
                  on.{editing ? " Leave empty to keep the current logo." : ""}
                </div>
              </div>
            </div>
          </div>
          ...
          <label
            style={{
              ...
              padding: "13px 16px",
              borderRadius: 10,
              border: `1.5px solid ${isActive ? "#2563eb40" : "#e2e8f0"}`,
          ...
          >
            ...
            <div>
              <div
                style={{ fontWeight: 700, color: "#0f172a", fontSize: 13.5 }}
              >
                Active
              </div>
              <div style={{ fontSize: 12, color: "#64748b" }}>
                Inactive brands stay on invoices already raised, but aren&apos;t
                offered on new ones.
              </div>
            </div>
          </label>
