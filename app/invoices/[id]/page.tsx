...
// messages show a time, older ones a date, so the Sales Chat thread here
// reads like every other chat surface in the app.
...
export default function InvoiceDetailPage() {
   ...
   * with neither is the guest/client writing in — never "mine" here.
   */
    ...
    <DashboardLayout title={invoice.invoice_number}>
      ...
      <div style={{ width: '100%', maxWidth: 'none' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #f1f5f9', padding: '24px 28px', marginBottom: 16 }}>
          ...
          {/* Payment progress — paid / remaining / percentage and the
              resolved money status. Same block and same figures the client
              sees on the share link and in their portal, so a conversation
              about "how much is left" can't start from two different numbers.
              The status pill above this card is the invoice LIFECYCLE
              (draft/sent/cancelled); this one is about the money. */}
          <div style={{ marginTop: 16, padding: '14px 16px', background: '#fafafa', border: '1px solid #f1f5f9', borderRadius: 10 }}>
            <PaymentProgressBar progress={progressOf(invoice)} currency={invoice.currency} />
          </div>
