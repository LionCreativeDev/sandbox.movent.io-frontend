...
export default function UserTasksPage() {
  ...
  useEffect(() => {
    ...
    // A Seller lands here too since 2026-09-04 (a PM can assign them a
    // task); the endpoints behind this page hard-scope them to their own
    // assigned tasks, so it renders as their "My Tasks" — no redirect, and
    // no canViewTasks needed either, which is why the module-permission
    // check below is what they fall through to.
  ...
  // …except for the roles that may never move a task to someone else, which
  // that bypass would otherwise cover (a worker is virtually always on the
  // team of their own task's project). Mirrors the same NON_REASSIGNING_ROLES
  // check update() enforces — see roleUtils.
  const iCanReassign = canRoleReassignTasks(me?.role_type);
                  ...
                  >
                    ...
                    <td style={{ padding: '13px 14px', fontSize: 12, color: '#64748b' }} onClick={e => e.stopPropagation()}>
                      {(() => {
                        ...
                        if (!iCanReassign || (!canEditTasks && !canAssignTasks && !isSelfTask && !isProjectTeamMemberFor(t))) {
