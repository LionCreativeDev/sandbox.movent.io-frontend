...
import {
  adminComplianceService, ComplianceCaseListItem, ComplianceCaseStatus, Paginated,
...
import ChatModal from '@/components/admin/compliance/ChatModal';
import CommentsModal from '@/components/admin/compliance/CommentsModal';
import TaskAttachmentsModal from '@/components/admin/compliance/TaskAttachmentsModal';
import ProjectAttachmentsModal from '@/components/admin/compliance/ProjectAttachmentsModal';
import ClientAttachmentsModal from '@/components/admin/compliance/ClientAttachmentsModal';
import InvoicesModal from '@/components/admin/compliance/InvoicesModal';
import FinalDeliveryModal from '@/components/admin/compliance/FinalDeliveryModal';
import ProjectOverviewModal from '@/components/admin/compliance/ProjectOverviewModal';
import {
...
// standalone /admin/compliance page (all projects) and a client's own
// compliance page (scoped via clientId). Caller owns DashboardLayout/guards;
// this only renders the content. Mirrors components/compliance/ProjectsListing.tsx
// (staff portal) — see that file for the shared design notes.
export default function ProjectsListing({
  ...
  const [page, setPage] = useState<Paginated<ComplianceCaseListItem> | null>(null);
  ...
  const loadCounts = async () => {
    try {
      const dashboard = await adminComplianceService.dashboard.get(clientId);
  ...
  const load = async (targetPage = 1) => {
    setLoading(true);
    try {
      const params: { status?: string; search?: string; client_id?: number; per_page?: number; page?: number } = { per_page: 20, page: targetPage };
      if (search) params.search = search;
      if (status) params.status = status;
      if (clientId) params.client_id = clientId;
      const res = await adminComplianceService.cases.list(params);
      setPage(res);
      setPageNum(res.current_page);
    } catch {
      toast.error('Failed to load compliance projects');
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadProjectZip = async (projectId: number, projectName: string) => {
    ...
    try {
      await adminComplianceService.cases.downloadProjectZip(projectId, `${projectName}-compliance-documents.zip`);
    } catch (err: unknown) {
  ...
  const downloadAllZip = async () => {
    ...
    try {
      await adminComplianceService.cases.downloadAllZip(`${zipFileNamePrefix}-compliance-documents.zip`, clientId);
    } catch (err: unknown) {
      toast.error(await blobErrorMessage(err, 'Failed to download documents'));
    } finally {
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#94a3b8' }}>
            ...
            {showClientLink && (
              <>
                {' '}
                <button onClick={() => router.push('/admin/compliance/clients')} style={{ background: 'none', border: 'none', padding: 0, color: '#2563eb', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
                  View by Client →
                </button>
              </>
            )}
                ...
                {cases.map((c, i) => (
                  <tr key={c.id} style={{ borderBottom: i < cases.length - 1 ? '1px solid #f8fafc' : 'none' }}>
                    ...
                    <td style={{ padding: '9px 8px' }} onClick={e => e.stopPropagation()}>
                      <RowAction
                        action="clientFiles"
                        // Both kinds of client-originated record the popup
                        // shows: files they sent, plus the receipts for what
                        // they paid. Counting only the first made a project
                        // with receipts but no chat files read as empty.
