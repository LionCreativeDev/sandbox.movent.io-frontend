...
import { adminLeadService, SalesDashboard } from '@/lib/services/adminLeadService';
...
import {
  ...
  HiDocumentText, HiBanknotes, HiClock, HiListBullet,
...
export default function SalesDashboardPage() {
  ...
  useEffect(() => {
    setLoading(true);
    adminLeadService.salesDashboard(year).then(setData).finally(() => setLoading(false));
  }, [year]);
  ...
  const { summary, monthly, sellers, invoice_summary } = data;
  const maxMonthly = Math.max(...monthly.map(m => m.total), 1);
  const winRate    = Math.round(summary.win_rate);
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', color: '#64748b', fontSize: 13 }}>Overview of your sales funnel and team performance</p>
          </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
            ...
            <StatCard icon={<HiCurrencyDollar size={20} />} label="Pending Revenue"       value={PKR(summary.pending_value)}  sub="Outstanding on won deals"           color="#d97706" />
            ...
            <StatCard icon={<HiExclamationCircle size={20} />} label="Overdue Follow-ups" value={summary.overdue_followups}  color="#ea580c"
              onClick={() => router.push('/admin/follow-ups')} />
            <StatCard icon={<HiChartBar size={20} />}       label="Win Rate"              value={`${winRate}%`}              sub={`${summary.won} of ${summary.won + summary.lost} closed`} color="#7c3aed" />
          </div>
        </div>

        {/* Invoice Summary — company-scoped (see the admin CompanySelector in
            the topbar; changing it re-fetches this whole dashboard, invoice
            summary included, from Api\Admin\SalesDashboardController). */}
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
            <StatCard icon={<HiListBullet size={20} />}       label="Total Invoices"     value={invoice_summary.total_count}            color="#0891b2" />
            <StatCard icon={<HiDocumentText size={20} />}     label="Total Invoiced"     value={PKR(invoice_summary.total_invoiced)}    color="#2563eb" />
            <StatCard icon={<HiBanknotes size={20} />}        label="Total Paid"         value={PKR(invoice_summary.total_paid)}        color="#059669" />
            <StatCard icon={<HiCurrencyDollar size={20} />}   label="Outstanding Amount" value={PKR(invoice_summary.total_outstanding)} color="#d97706" />
            <StatCard icon={<HiExclamationCircle size={20} />} label="Overdue Invoices"  value={invoice_summary.overdue_count}          color="#dc2626" />
            <StatCard icon={<HiClock size={20} />}              label="Partially Paid Invoices" value={invoice_summary.partially_paid_count} color="#7c3aed" />
          </div>
        </div>

        {/* Monthly chart — full width */}
        <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #f1f5f9', padding: '20px 24px', marginBottom: 16 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f8fafc' }}>
                  {['Seller', 'Total', 'Open', 'Won', 'Lost', 'Win Rate', 'Won Value', 'Pending Value'].map(h => (
                ...
                {sellers.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id ?? idx} style={{ borderTop: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '12px 16px', fontSize: 13, fontWeight: 600, color: '#d97706' }}>{PKR(s.pending_value)}</td>
                    </tr>
                  );
                })}
              </tbody>
