...
import { inp, lbl, card, Badge, TASK_SC, PRIORITY_SC, fmtDate, ALLOWED_ATTACHMENT_TYPES, fmtFileSize, asRelation, DRAFT_HINT, DraftNotice } from '@/components/admin/projects/shared';
...
// per current policy — only a Client portal account is excluded outright.
// Sellers became assignable on 2026-09-04 and are the one candidate NOT
// filtered by project-team membership (they never get a
// project_team_members row) — see the users filter below.
const NEVER_TASK_ASSIGNEE_ROLES = ['client'];

// Which of this admin's users belong to a given company — a Seller is offered
// on the strength of this alone, so it has to be right: `company_assignments`
// covers a multi-company Seller, `company` their own primary row.
const belongsToCompany = (u: User, companyId: number | null): boolean =>
  companyId != null && (
    (u.company_assignments ?? []).some(a => a.company_id === companyId)
    || u.company?.id === companyId
  );

// "Which company's Seller is this" — the reason the label is shown at all:
// an admin owning several companies must see whose Seller they're handing
// the task to. Prefers the assignment matching this project's company.
const companyLabelFor = (u: User, companyId: number | null): string | null =>
  (u.company_assignments ?? []).find(a => a.company_id === companyId)?.company_name
  ?? u.company?.name
  ?? null;
...
export default function ProjectTasksPage() {
  ...
  // This project's own company — scopes the Seller options below (an admin's
  // user list spans every company they own, and team membership, which
  // implicitly did this scoping for every other candidate, is exactly what
  // Sellers are exempt from).
  ...
  useEffect(() => {
    ...
    adminProjectService.getOne(projectId).then(p => {
      ...
      // Timesheets) — with Sellers as the deliberate exception, offered
      // company-wide since they never hold a project_team_members row.
      // Still sourced from userService.list() (not project.team_members
      // directly) since the "no Project Management access" warning below
      // needs each user's full company_assignments, which team_members'
      // nested user object doesn't carry. A Client portal account is never
      // a task assignee.
      const teamMemberIds = new Set((p.team_members ?? []).map(tm => tm.user_id));
      setProjectCompanyId(p.company_id ?? null);
      userService.list().then(d => setUsers(d.users.filter(u => {
        if (!u.is_active || NEVER_TASK_ASSIGNEE_ROLES.includes(u.role_type ?? '')) return false;
        // Sellers are offered company-wide ("any seller", 2026-09-04) since
        // they never appear in team_members; everyone else still has to be
        // on this project's team.
        return u.role_type === 'seller'
          ? belongsToCompany(u, p.company_id ?? null)
          : teamMemberIds.has(u.id);
      }))).catch(() => {});
    }).catch(() => {});
  }, [projectId]); // eslint-disable-line react-hooks/exhaustive-deps

  // ?new=1 — arriving from the cross-project Tasks page's "Create Task" flow
  // (app/admin/tasks/page.tsx), which picks a project and then needs to land
  // ON the form, not on this project's task list. Read straight off the URL
  // rather than via useSearchParams(), which would pull this whole client
  // tree behind a Suspense boundary just to answer one boolean. The param is
  // then stripped so a later reload/back doesn't silently reopen the form.
  useEffect(() => {
              ...
              <select value={form.assigned_to} onChange={e => setF('assigned_to', e.target.value)} style={inp}>
                ...
                {users.map(u => (
                  <option key={u.id} value={u.id}>
                    {u.name} ({roleDisplayLabel(u)})
                    {/* Whose Seller this is — the company matters here because
                        Sellers are offered company-wide rather than off this
                        project's team. A Seller needs no Project Management
                        access for an assigned task: their own-tasks view is
                        role-derived, not permission-gated. */}
                    {u.role_type === 'seller'
                      ? (companyLabelFor(u, projectCompanyId) ? ` — ${companyLabelFor(u, projectCompanyId)}` : '')
                      : (hasProjectManagementAccess(u) ? '' : ' — no Project Management access')}
                  </option>
                ))}
              </select>
            </div>
          </div>
          {/* Never shown for a Seller: they reach a task assigned to them
              through a role-derived path in Api\User\TaskController (own
              assigned tasks only), so "no Project Management access" would
              be a false warning. */}
          {assignedUser && assignedUser.role_type !== 'seller' && !hasProjectManagementAccess(assignedUser) && (
