...
// The Super Admin's own account rather than something they manage, so it sits
// under its own heading at the bottom instead of among the tenant screens.
const ACCOUNT_ITEMS = [
  { href: '/super-admin/profile', icon: HiUserCircle, label: 'Account Settings' },
];

const groupHeading: React.CSSProperties = {
  color: '#64748b', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em',
...
export default function SuperAdminSidebar() {
  ...
  // One renderer for both groups — the hover/active styling is inline, so a
  // second copy of it would drift the moment either is touched.
  const renderItem = ({ href, icon: Icon, label }: typeof NAV_ITEMS[number]) => {
  ...
  return (
    <aside style={{
      ...
      <nav style={{ padding: '16px 12px', flex: 1 }}>
        <div style={groupHeading}>Management</div>
        {NAV_ITEMS.map(renderItem)}

        <div style={{ ...groupHeading, paddingTop: 18 }}>Account</div>
